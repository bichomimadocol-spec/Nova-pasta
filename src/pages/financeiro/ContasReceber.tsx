...
import { TituloFinanceiro, BaixaTitulo, Cliente, ContaBancaria, MovimentoConta } from '../../App';
...
export default function ContasReceberPage({ 
    ...
    return titulos
      ...
      .filter(t => {
        ...
        if (searchTerm) {
          ...
          const clienteNome = t.pessoaNome || clientes.find(c => c.id === t.pessoaId)?.nome || '';
          return (
  ...
  }, [titulos, filterStatus, searchTerm, clientes]);
  ...
  const handleSaveTitulo = () => {
    ...
    const cliente = clientes.find(c => c.id === form.pessoaId);
    ...
    const novo: TituloFinanceiro = {
      ...
      pessoaNome: cliente?.nome || 'Cliente Avulso',
  ...
  const handleConfirmBaixa = () => {
    ...
    const novoMovimento: MovimentoConta = {
      ...
      tipo: 'ENTRADA',
      valor: Number(baixaForm.valor),
      historico: `Recebimento: ${selectedTitulo.descricao}`,
      referenciaTipo: 'AR',
      referenciaId: selectedTitulo.id,
      conciliado: true, // Auto-reconcile for now
      observacao: `Cliente: ${selectedTitulo.pessoaNome}`
    };
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl w-[500px]">
            <h2 className="text-xl font-bold text-gray-800 mb-4">Novo Título a Receber</h2>
            <div className="space-y-4">
              ...
              <div>
                ...
                <select 
                  ...
                  className="w-full border rounded p-2"
                >
                  <option value={0}>Cliente Avulso / Não Cadastrado</option>
                  {clientes.map(c => (
      ...
      {showBaixaModal && selectedTitulo && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl w-[500px]">
            <h2 className="text-xl font-bold text-gray-800 mb-4">Recebimento de Título</h2>
            
            <div className="bg-gray-50 p-4 rounded mb-4 text-sm">
              <p><strong>Cliente:</strong> {selectedTitulo.pessoaNome}</p>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleConfirmBaixa} className="flex-1 px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 font-bold">Confirmar Recebimento</button>
            </div>
          </div>
        </div>
      )}
